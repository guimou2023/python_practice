...
def make_transaction(log_obj,account_data,tran_type,amount,**others):
    ...
    if tran_type in  settings.TRANSACTION_TYPE:
        ...
        if settings.TRANSACTION_TYPE[tran_type]['action'] == 'plus':
            ...
            logger.logger('bills', account_data['id'], '还款','转入：%s' %amount)
        elif settings.TRANSACTION_TYPE[tran_type]['action'] == 'in':
            new_balance = old_balance + amount + interest
            ...
            if  new_balance <0:
                ...
                return
            if not tran_type == 'consume':
                logger.logger('bills', account_data['id'], '取款','取款：%s 账户扣除%s' %(amount,amount+interest))
        elif settings.TRANSACTION_TYPE[tran_type]['action'] == 'out':
            new_balance = old_balance - amount - interest
            #check credit
            if  new_balance <0:
                print('''\033[31;1mYour credit [%s] is not enough for this transaction [-%s], your current balance is
